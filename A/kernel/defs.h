...
int             load_exec_page(struct proc *, uint64, struct inode *);
int             is_valid_address(struct proc *, uint64);
struct page_info* find_victim_page(struct proc *);
...
int             read_from_swap(struct proc *, uint64, int, uint64);

// pagefault.c
int             handle_page_fault(struct proc *p, uint64 fault_addr, int scause);

// pagefault_lru.c (Bonus: LRU-WSP)
