...
// Global timestamp counter for LRU tracking (bonus)
uint64 global_timestamp = 0;
struct spinlock timestamp_lock;
...
found:
  ...
  for(int i = 0; i < MAX_PROC_PAGES; i++) {
    ...
    p->pages[i].fifo_seq = 0;
    p->pages[i].swap_slot = -1;
  }
  
  // Initialize swap management
  p->swap_file = 0;
  p->swap_path[0] = 0;
...
freeproc(struct proc *p)
{
  ...
  if(p->swap_file) {
    ...
    if(p->swap_path[0]) {
      begin_op();
      struct inode *swap_ip = namei(p->swap_path);
      if(swap_ip) {
        ilock(swap_ip);
        swap_ip->nlink = 0;
        iupdate(swap_ip);
...
kexit(int status)
{
  ...
  // Close swap file and clean up swap
  if(p->swap_file) {
    int slots_freed = 0;
    for(int i = 0; i < 1024; i++) {
      if(p->swap_slots[i] != 0) {
